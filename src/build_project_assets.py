...
for idx, e in enumerate(employee, start=1):
    for note in e["notes"]:
        note_data.append([idx, e["name"], note])

notes = pd.DataFrame(
    note_data, columns=["employee_id", "employee_name", "note"]
).assign(event_date=np.random.choice(df.event_date, size=len(note_data), replace=True))
...
team_map = {team: random.choice(managers) for team in df.team_id.unique()}
...
y = (
    X.join(
        ...
        .set_index("employee_id")[["recruited"]]
    )
    .recruited
)
